...
*** Generate dummy variables for attack_type ;
drop attack_typenorm attack_typedos ;
label F_attack_type = 'From: attack_type' ;
length F_attack_type $ 5;
F_attack_type = put( attack_type , $5. );
%DMNORMIP( F_attack_type )
if missing( attack_type ) then do;
   attack_typenorm = .;
   attack_typedos = .;
end;
else do;
   if F_attack_type = 'DOS'  then do;
      attack_typenorm = 0;
      attack_typedos = 1;
   end;
   else if F_attack_type = 'NORM'  then do;
      attack_typenorm = 1;
      attack_typedos = 0;
   end;
   else do;
...
END;
*** *****************************;
*** Writing the Residuals  of the Node attack_type ;
*** ******************************;
IF MISSING( attack_typenorm ) THEN R_attack_typenorm  = . ;
ELSE R_attack_typenorm  = attack_typenorm  - P_attack_typenorm ;
IF MISSING( attack_typedos ) THEN R_attack_typedos  = . ;
ELSE R_attack_typedos  = attack_typedos  - P_attack_typedos ;
*** *************************;
